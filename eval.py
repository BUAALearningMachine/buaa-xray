...
from data.SIXray import SIXray_CLASSES as labelmap, SIXrayDetection, BaseTransform, SIXrayAnnotationTransform, \
    TEST_SET_PATH
...
parser.add_argument('--trained_model', default='weights/ssd300_xray_20191130_hyw_6000_24000.pth', type=str, help='Trained state_dict file path to open')
...
parser.add_argument('--dataset_root', default=TEST_SET_PATH, help='Dataset root directory path')
...
annopath = os.path.join('%s' % args.dataset_root, 'Annotation', 'battery_%s.txt')
imgpath = os.path.join('%s' % args.dataset_root, 'Image', 'battery_%s.txt')
...
def do_python_eval(output_dir='output'):
    ...
    for i, cls in enumerate(labelmap):
        filename = get_voc_results_file_template(set_type, cls)
        rec, prec, ap = voc_eval(filename, annopath, args.imagesetfile, cls, cachedir)
        aps += [ap]
...
def voc_eval(detpath, annopath, imagesetfile, classname, cachedir, ovthresh=0.5):
    ...
    if not os.path.isfile(cachefile):
        # load annots
        recs = {}
        for i, image_name in enumerate(imagenames):
            recs[image_name] = parse_rec(annopath % image_name)
            if i % 100 == 0:
